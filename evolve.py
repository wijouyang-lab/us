...
系统的核心选股哲学是【产业链逻辑驱动】：从宏观事件提炼产业链主线，找到二级受益标的，技术面只作为入场时机确认，不是选股的主要依据。当前系统对 Top 1-5 标的进行展开式详细分析（产业链逻辑、个股新闻、技术确认三个维度都要详细），观察池覆盖 Rank 6-12。
...
3. 可以调整的是：技术确认的阈值（RSI、乖离率门槛）、新闻排雷的严格程度、止损止盈的计算方式
4. 不要改变代码的整体结构、API调用方式、邮件发送逻辑、入库逻辑
